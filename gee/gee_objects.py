...
class ERA5Daily:
    def __init__(self, farm, aoi):
        ...
        self.farm = farm
        self.aoi = aoi
        self.gee_image_collection = (ee.ImageCollection('ECMWF/ERA5_LAND/DAILY_AGGR').filterBounds(aoi)
    ...
    def stats_calculator(self, indexes, scale=10, crs='EPSG:4326'):
        dates = self.get_feature('date', unique=True)

        data_stats = [calculate_stats_geeimage(self.get_image(i), self.aoi, indexes, scale, crs) for i in dates]
...
class Sentinel2:
    def __init__(self, farm, aoi):
        ...
        self.farm = farm
        self.aoi = aoi
        self.gee_image_collection = (ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED').filterBounds(aoi)
                                     .map(lambda img: img.set('timestamp', img.date().format('YYYY-MM-dd HH:mm:ss')))
                                     .map(lambda img: img.set('date', img.date().format('YYYY-MM-dd'))))
    ...
    def count_pixels_features(self, index='B4', scale=10, crs='EPSG:4326'):
        ...
        data_stats = [calculate_stats_geeimage(self.get_image(i), self.aoi, index, scale, crs, reducer='count') for i in dates]
        data_stats = pd.concat(data_stats)
